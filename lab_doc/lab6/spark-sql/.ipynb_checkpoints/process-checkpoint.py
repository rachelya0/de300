...
def main():
    ...
    # select rows with 'age' between 30 and 50 (inclusive)
    filtered_data = data.filter((col("age") >= 30) & (col("age") <= 50))
    
    # transform the selected PySpark DataFrame into a Pandas DataFrame
    pandas_df = filtered_data.toPandas()
    
    # print out the summary statistics using 'describe()'
